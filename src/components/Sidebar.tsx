...
const Sidebar: React.FC = () => {
        ...
        >
          <div className="flex items-center flex-1 min-w-0">
            {!isSidebarCollapsed && hasChildPages ? (
              <button
                className="p-0.5 rounded hover:bg-gray-200 dark:hover:bg-slate-700 mr-1 flex-shrink-0"
                onClick={(e) => {
                  e.stopPropagation();
              ...
              <>
                {isEditing ? (
                  <input
                    ...
                    className="flex-1 px-1 py-0.5 text-sm bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-600 rounded focus:outline-none focus:ring-2 focus:ring-purple-500 text-gray-900 dark:text-white"
                    autoFocus
                    onClick={(e) => e.stopPropagation()}
                  />
                ) : (
                  <span 
                    className="truncate text-sm hover:bg-gray-100 dark:hover:bg-slate-800 px-1 py-0.5 rounded flex-1"
                    onClick={(e) => {
                      e.stopPropagation();
            ...
            {!isSidebarCollapsed && !isEditing && (
              <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button
                  ...
                  title="Delete page"
                  className="p-1 rounded hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-400 hover:text-red-500"
                >
                  <Trash2 size={12} />
                </button>
              </div>
  ...
  return (
    <div className={`${isSidebarCollapsed ? 'w-16' : 'w-64'} bg-gray-50 dark:bg-slate-900 border-r border-gray-200 dark:border-slate-700 h-screen flex flex-col transition-all duration-300 ease-in-out`}>
      {/* Workspace Header */}
      ...
      <div className="flex-1 overflow-y-auto p-2">
        ...
        <div className="mb-4">
          <div
            className="flex items-center py-1 px-2 rounded-md cursor-pointer text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800 hover:text-gray-900 dark:hover:text-white transition-colors"
            onClick={() => !isSidebarCollapsed && setIsTemplatesExpanded(!isTemplatesExpanded)}
            title={isSidebarCollapsed ? 'Templates' : undefined}
          >
            {!isSidebarCollapsed && (
              <button className="p-0.5 rounded hover:bg-gray-200 dark:hover:bg-slate-700 mr-1 flex-shrink-0">
                {isTemplatesExpanded ? (
                  <ChevronDown size={12} />
                ) : (
                  <ChevronRight size={12} />
            ...
            <Folder size={14} className="mr-2 flex-shrink-0 text-gray-500 dark:text-gray-400" />
            {!isSidebarCollapsed && (
        ...
        <div>
          {!isSidebarCollapsed && (
            <div className="flex items-center py-1 px-2 mb-2">
              <span className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide font-medium">Pages</span>
            </div>
          )}
